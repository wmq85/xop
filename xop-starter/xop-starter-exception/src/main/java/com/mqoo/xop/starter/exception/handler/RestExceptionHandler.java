...
package com.mqoo.xop.starter.exception.handler;
 ...
 *
 * <ol>
 * <li>Upon encountering an Exception, the configured {@link RestErrorResolver} is consulted to
 * resolve the exception into a {@link RestError} instance.</li>
 * <li>The HTTP Response's Status Code will be set to the {@code RestError}'s
 * {@link RestError#getStatus() status} value.</li>
 * <li>The {@code RestError} instance is presented to a configured {@link RestErrorConverter} to
 * allow transforming the {@code RestError} instance into an object potentially more suitable for
 * rendering as the HTTP response body.</li>
 * <li>The {@link #setMessageConverters(org.springframework.http.converter.HttpMessageConverter[])
 * HttpMessageConverters} are consulted (in iteration order) with this object result for rendering.
 * The first {@code HttpMessageConverter} instance that
 * {@link HttpMessageConverter#canWrite(Class, org.springframework.http.MediaType) canWrite} the
 * object based on the request's supported {@code MediaType}s will be used to render this result
 * object as the HTTP response body.</li>
 * <li>If no {@code HttpMessageConverter}s {@code canWrite} the result object, nothing is done, and
 * this handler returns {@code null} to indicate other ExceptionResolvers potentially further in the
 * resolution chain should handle the exception instead.</li>
 * </ol>
 *
 * <h3>Defaults</h3> This implementation has the following property defaults:
 * <table>
 * <tr>
 * <th>Property</th>
 * <th>Instance</th>
 ...
 * <td>Converts {@link RestError} instances to {@code java.util.Map} instances to be used as the
 * response body. Maps can then be trivially rendered as JSON by a (configured)
 * {@link HttpMessageConverter HttpMessageConverter}. If you want the raw {@code RestError} instance
 * to be presented to the {@code HttpMessageConverter} instead, set this property to
 * {@code null}.</td>
 * </tr>
 * <tr>
 ...
 * "http://static.springsource.org/spring/docs/current/spring-framework-reference/html/mvc.html#mvc-config-enable">defined
 * here</a> (specifically item #5)</td>
 * </tr>
 * </table>
 *
 * <h2>JSON Rendering</h2> This implementation comes pre-configured with Spring's typical default
 * {@link HttpMessageConverter} instances; JSON will be enabled automatically if Jackson is in the
 * classpath. If you want to match the JSON representation shown in the article above (recommended)
 * but do not want to use Jackson (or the Spring's default Jackson config), you will need to
...
public class RestExceptionHandler extends AbstractHandlerExceptionResolver
     ...
     * @return a corresponding ModelAndView to forward to, or <code>null</code> for default
     *         processing
     */
    @Override
    protected ModelAndView doResolveException(HttpServletRequest request,
        ...
        log.error("Start to handle exception.", ex);
        ...
        String requestId = requestIdResolver.resolveRequestId(webRequest, handler, ex);
        ...
        try {
            ...
            log.error("Acquiring ModelAndView for Exception [" + ex + "] resulted in an exception.",
                    invocationEx);
        }
    ...
    protected ModelAndView getModelAndView(ServletWebRequest webRequest, Object handler,
        ...
        Object body = error; // default the error instance in case they don't configure an error
                             // converter
